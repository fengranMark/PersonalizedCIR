            ...
            if not args.use_PRF:
                ...
                if len(rel_label) > 0:
                    if args.PRF_mode == "hard":
                        if args.level == "token":
                            token_set = [] # for token-level expansion
                            ...
                            for j in range(len(rel_label_2)):
                                if rel_label_2[j] == 1:
                                    conv_query.append(query_tokenizer.convert_tokens_to_ids(token_set[j]))
                            conv_query = conv_query[:args.max_concat_length - 1]
                            conv_query.append(query_tokenizer.sep_token_id)
    ...
    def __init__(self, args, query_tokenizer, filename):
        ...
        for i in trange(n):
            ...
            if 'id' in data[i]:
                ...
                sample_id = data[i]['sample_id']
            sample_id = sample_id.replace('-', '_') # for cast21
            if 'output' in data[i]:
            ...
            if args.eval_type == "answer":
                ...
                data_2[i] = json.loads(data_2[i])
                rewrite = data_2[i]['rewrite'][0] + ' ' + data_2[i]['response'][0]
            elif args.eval_type == "fusion":
                data_2[i] = json.loads(data_2[i])
                rewrite = rewrite
                answer = data_2[i]['answer_utt_text']
    ...
    def get_collate_fn(args):
        ...
        def collate_fn(batch: list):
            ...
            for example in batch:
                # padding
                #answer, answer_mask = pad_seq_ids_with_mask(example.answer, max_length = args.max_query_length)
                rewrite, rewrite_mask = pad_seq_ids_with_mask(example.rewrite, max_length = args.max_query_length)
                bt_sample_id.append(example.sample_id)
...
class ConvExample_cast23_rewrite:
    def __init__(self, sample_id, 
                       rewrite,
                       answer):
        self.sample_id = sample_id
...
class test_cast23_rewrite(Dataset):
    def __init__(self, args, query_tokenizer, filename):
        self.examples = []
        ...
        for i in trange(n):
            ...
            if 'id' in data[i]:
                ...
                sample_id = data[i]['sample_id']
            #if 'rewrite_utt_text' not in data[i]:
            #    rewrite = data[i]['oracle_utt_text']
            if 'fusion' in data[i]:
                rewrite = data[i]['fusion']
            else:
                rewrite = data[i]['rewrite']
            if 'response_utt_text' in data[i]:
                answer = data[i]['response_utt_text']
            ...
            self.examples.append(ConvExample_cast23_rewrite(sample_id,
                                        rewrite,
                                        answer,
                                        )) 
    ...
    def get_collate_fn(args):
        ...
        def collate_fn(batch: list):
            ...
            for example in batch:
                # padding
                answer, answer_mask = pad_seq_ids_with_mask(example.answer, max_length = args.max_response_length)
                rewrite, rewrite_mask = pad_seq_ids_with_mask(example.rewrite, max_length = args.max_query_length)
                bt_sample_id.append(example.sample_id)
                bt_rewrite.append(rewrite)
                bt_rewrite_mask.append(rewrite_mask)   
...
class ConvExample_cast23_mixeval:
    def __init__(self, sample_id, 
                       rewrite,
                       answer):
        self.sample_id = sample_id
...
class test_cast23_mixeval(Dataset):
    def __init__(self, args, query_tokenizer, filename):
        self.examples = []
        ...
        if args.is_mixeval:
            with open(args.test_file_path_2, 'r') as f:
                data_2 = f.readlines()
        ...
        for i in trange(n):
            ...
            if args.is_mixeval:
                data_2[i] = json.loads(data_2[i])
                if len(data[i]['ptkb_provenance']) == 0:
                    data[i] = data_2[i]
            answer = None
            if 'id' in data[i]:
                sample_id = data[i]['id']
            ...
            self.examples.append(ConvExample_cast23_mixeval(sample_id,
                                        rewrite,
                                        answer,
                                        )) 
    ...
    def __init__(self, args, tokenizer, filename):
        ...
        for line in tqdm(data):
            ...
            if args.collate_fn_type == "flat_concat_for_train":
                ...
                oracle_labels[oracle_labels == tokenizer.pad_token_id] = -100 # T5 for -100
                #oracle_labels[oracle_labels == tokenizer.pad_token_id] = 0 # ance for 0
                ...
                #answer_labels[answer_labels == tokenizer.pad_token_id] = 0
...
class T5RewriterDataset_cast21(Dataset):
    def __init__(self, args, tokenizer, filename):
        self.examples = []
...
class T5RewriterDataset_cast23(Dataset):
    def __init__(self, args, tokenizer, filename):
        self.examples = []
...
class T5RewriterDataset_allcast(Dataset):
    def __init__(self, args, tokenizer, root):
        self.examples = []

        with open(root + "/cast19/eval_topics.jsonl", encoding="utf-8") as f:
            data1 = f.readlines()
        with open(root + "/cast20/eval_topics.jsonl", encoding="utf-8") as f:
            data2 = f.readlines()
        with open(root + "/cast21/eval_topics.json", encoding="utf-8") as f:
            data3 = f.readlines()
        with open(root + "/cast22/eval_topics.jsonl", encoding="utf-8") as f:
            data4 = f.readlines()
...
class ConvExample_LLM4CS:
    def __init__(self, sample_id,
                       rewrite_list, 
                       response_list):
        self.sample_id = sample_id
        self.rewrite_list = rewrite_list
        self.response_list = response_list


class ConvDataset_LLM4CS(Dataset):
    def __init__(self, args, query_tokenizer, filename):
        self.examples = []
        ...
        for i in trange(n):
            ...
            data[i] = json.loads(data[i])
            sample_id = data[i]['sample_id']
            rewrite_list = [query_tokenizer.encode(j, add_special_tokens=True) for j in data[i]['predicted_rewrite']]
            response_list = [query_tokenizer.encode(j, add_special_tokens=True) for j in data[i]['predicted_response']]




            self.examples.append(ConvExample_LLM4CS(sample_id,
                                            rewrite_list, 
                                            response_list,
                                            ))          
    ...
    def get_collate_fn(args):
        ...
        def collate_fn(batch: list):
            ...
            for example in batch:
                # padding
                print(example.rewrite_list)
                q1, q1_mask = pad_seq_ids_with_mask(example.rewrite_list[0], max_length = args.max_concat_length)
                q2, q2_mask = pad_seq_ids_with_mask(example.rewrite_list[1], max_length = args.max_concat_length)
                q3, q3_mask = pad_seq_ids_with_mask(example.rewrite_list[2], max_length = args.max_concat_length)
                q4, q4_mask = pad_seq_ids_with_mask(example.rewrite_list[3], max_length = args.max_concat_length)
                q5, q5_mask = pad_seq_ids_with_mask(example.rewrite_list[4], max_length = args.max_concat_length)
                r1, r1_mask = pad_seq_ids_with_mask(example.response_list[0], max_length = args.max_concat_length)
                r2, r2_mask = pad_seq_ids_with_mask(example.response_list[1], max_length = args.max_concat_length)
                r3, r3_mask = pad_seq_ids_with_mask(example.response_list[2], max_length = args.max_concat_length)
                r4, r4_mask = pad_seq_ids_with_mask(example.response_list[3], max_length = args.max_concat_length)
                r5, r5_mask = pad_seq_ids_with_mask(example.response_list[4], max_length = args.max_concat_length)


                bt_sample_id.append(example.sample_id)
                bt_q1_ids.append(q1)
                bt_q1_mask.append(q1_mask)
                bt_q2_ids.append(q2)
                bt_q2_mask.append(q2_mask)
                bt_q3_ids.append(q3)
                bt_q3_mask.append(q3_mask)
                bt_q4_ids.append(q4)
                bt_q4_mask.append(q4_mask)
                bt_q5_ids.append(q5)
                bt_q5_mask.append(q5_mask)
                bt_r1_ids.append(r1)
                bt_r1_mask.append(r1_mask)
                bt_r2_ids.append(r2)
                bt_r2_mask.append(r2_mask)
                bt_r3_ids.append(r3)
                bt_r3_mask.append(r3_mask)
                bt_r4_ids.append(r4)
                bt_r4_mask.append(r4_mask)
                bt_r5_ids.append(r5)
                bt_r5_mask.append(r5_mask)




            collated_dict["bt_sample_id"] = bt_sample_id
            collated_dict["bt_q1_ids"] = bt_q1_ids
            collated_dict["bt_q1_mask"] = bt_q1_mask
            collated_dict["bt_q2_ids"] = bt_q2_ids
            collated_dict["bt_q2_mask"] = bt_q2_mask
            collated_dict["bt_q3_ids"] = bt_q3_ids
            collated_dict["bt_q3_mask"] = bt_q3_mask
            collated_dict["bt_q4_ids"] = bt_q4_ids
            collated_dict["bt_q4_mask"] = bt_q4_mask
            collated_dict["bt_q5_ids"] = bt_q5_ids
            collated_dict["bt_q5_mask"] = bt_q5_mask
            collated_dict["bt_r1_ids"] = bt_r1_ids
            collated_dict["bt_r1_mask"] = bt_r1_mask
            collated_dict["bt_r2_ids"] = bt_r2_ids
            collated_dict["bt_r2_mask"] = bt_r2_mask
            collated_dict["bt_r3_ids"] = bt_r3_ids
            collated_dict["bt_r3_mask"] = bt_r3_mask
            collated_dict["bt_r4_ids"] = bt_r4_ids
            collated_dict["bt_r4_mask"] = bt_r4_mask
            collated_dict["bt_r5_ids"] = bt_r5_ids
            collated_dict["bt_r5_mask"] = bt_r5_mask
